...
const AdminBookings = () => {
  ...
  const fetchBookings = async () => {
    ...
    const { data, error } = await supabase
      .from('bookings')
      .select('id, booking_date, start_time, status, total_price, shops(name), profiles:customer_id(full_name, phone), barbers(name), services(name')
                ...
                <div className="flex items-start justify-between">
                  <div className="space-y-1.5">
                    ...
                    <p className="text-sm text-muted-foreground">
                      ...
                      {b.booking_date} · {b.start_time?.slice(0, 5)} · {b.shops?.name || 'Unknown Shop'} · {b.barbers?.name || 'Unknown Stylist'}
                    </p>
